...
import type { CreateCountryFiscalRuleRequest } from "../../types/countryFiscalRule.types"
...
export function CreateCountryModal({
  ...
  function handleCountryChange(value: string) {
    const selected = internationalizationOptions.find(
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (normalizedFields.length === 0) {
        setError(t("countries.requiredFieldError"))
        setLoading(false)
        return
      }

      const payload: CreateCountryFiscalRuleRequest = {
      ...
      await CountryFiscalRuleService.create(payload)

      onCreated()
      onClose()
    } catch {
      setError(t("countries.createError"))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="w-full max-w-[720px] max-h-[92vh] overflow-y-auto app-scrollbar surface-premium rounded-2xl p-5">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <h2 className="text-xl font-bold mt-1">
              {t("countries.newFiscalCountry")}
            </h2>

            <p className="text-muted text-sm mt-1">
              {t("countries.createDescription")}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          >
            {loading ? t("countries.saving") : t("countries.saveFiscalCountry")}
          </button>
        </form>
      </div>
    </div>
