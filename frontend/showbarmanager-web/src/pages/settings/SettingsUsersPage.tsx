...
function getUserLanguageCode(user: User) {
  const userWithLanguage = user as UserWithLanguage

  return userWithLanguage.language || user.languageCode || null
}

function getFlag(countryCode: string | null | undefined) {
...
function getPhoneDisplay(user: User) {
  if (!user.phone) {
    return "-"
  }

  const flag = getFlag(user.phoneCountryCode)
  const dialCode = user.phoneDialCode || ""

  if (flag || dialCode) {
    return `${flag} ${dialCode} ${user.phone}`.trim()
  }
...
export function SettingsUsersPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return data.filter((user) => {
      ...
      const matchesSearch =
        ...
        (getUserLanguageCode(user) || "").toLowerCase().includes(searchTerm)
  ...
  function handleRefresh() {
    refetch()
      .then(() => {
        toast.success(t("users.listUpdated"))
      })
      .catch(() => {
  ...
  function buildUpdatePayload(user: User, active: boolean): UpdateUserPayload {
    return {
      name: user.name,
      email: user.email,
      phone: user.phone ?? null,
      phoneCountryCode: user.phone ? user.phoneCountryCode ?? null : null,
      phoneDialCode: user.phone ? user.phoneDialCode ?? null : null,
      role: user.roles?.[0] || "OPERATOR",
      active,
      language: getUserLanguageCode(user),
  ...
  async function handleToggleActive(user: User) {
    ...
    const confirmed = window.confirm(
      nextActive ? t("users.activateUserConfirm") : t("users.deactivateUserConfirm")
    )
    ...
    try {
      setActiveLoadingId(user.id)
      await UserService.update(user.id, buildUpdatePayload(user, nextActive))
      await refetch()
      toast.success(nextActive ? t("users.userActivatedSuccess") : t("users.userDeactivatedSuccess"))
    } catch {
  ...
  async function handleDelete(user: User) {
    ...
    if (user.masterUser) {
      toast.warning(t("users.masterDeleteBlocked"))
      return
    }

    const confirmed = window.confirm(t("users.deleteUserConfirm"))
  ...
  return (
    <div className="space-y-4">
      ...
      {!isLoading && !isError && (
        <section className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-3">
          {filteredUsers.map((user) => (
            <article
              key={user.id}
              className="surface-premium rounded-2xl p-3 hover:border-primary/50 transition"
            >
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <InfoBox label={t("common.phone", "Telefone")} value={getPhoneDisplay(user)} />
              ...
              <div className="mt-3 flex items-center justify-between gap-3 border-t border-border pt-3">
                ...
                {canManageUsers() && (
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      onClick={() => {
                        handleToggleActive(user).catch(() => {
                          toast.error(t("messages.unexpectedError"))
                        })
                      }}
                      disabled={activeLoadingId === user.id}
                      className="w-8 h-8 rounded-full bg-background border border-border flex items-center justify-center hover:border-primary hover:text-primary transition disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <CreateUserModal
        ...
        onCreated={() => {
          refetch().catch(() => {
            toast.warning(t("users.createdRefreshError"))
          })
        }}
      />
      ...
      <EditUserModal
        ...
        onUpdated={() => {
          refetch().catch(() => {
            toast.warning(t("users.updatedRefreshError"))
          })
        }}
      />
    </div>
...
function InfoBox({
  ...
  return (
    <div className="surface-muted rounded-2xl p-2.5 min-w-0">
      <p className="text-[11px] uppercase tracking-wide text-muted">
        {label}
      </p>
...
function AccessBox({
  ...
  return (
    <div className="mt-2.5 surface-muted rounded-2xl p-2.5">
      <p className="text-[11px] uppercase tracking-wide text-muted mb-2">
        {title}
      </p>

      <div className="flex flex-wrap gap-1.5 max-h-16 overflow-y-auto app-scrollbar pr-1">
        {items.length > 0 ? (
          items.map((item) => (
            <span
              key={item}
              className="inline-flex items-center gap-1 bg-primarySoft text-primary px-2 py-0.5 rounded-full text-[11px] font-medium"
            >
              <ShieldCheck size={12} />
              {item}
            </span>
